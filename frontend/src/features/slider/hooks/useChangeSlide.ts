...
//  доработать!!!!!!!
  ...
  const isRightArrow = !(infiniteLoop && (slides.length > quantityShowSlides)) && 
...
//  доработать!!!!!!!
  const preparedIndexesForPag = useMemo(() => {
    const currentIndexesPag = getPagIndexes(
      ...
      (infiniteLoop && (slides.length > quantityShowSlides)) ? stateSlider.indexSlide - 1 : stateSlider.indexSlide,
