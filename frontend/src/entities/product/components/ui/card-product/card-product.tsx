...
export const CardProductUI = ({
  ...
  return <article className={styles['card-product']}>
          <div className={styles['card-images-container']}>
            ...
            <div className={styles['card-icons-actions']}>
                <div className={clsx(isNew ? styles['card-is-new'] : styles['card-is-old'])}>
                  {isNew ? <span>new</span> : ''}
                </div>
          ...
          <div className={styles['card-product-descrip']}>
            <div className={styles['card-product-prices']}>
              { oldPrice ? (
                <>
                  <div className={clsx(styles['new-price'],styles['price'])}>{price} {`${currencyType}`}</div>  
                  <div className={clsx(styles['old-price'],styles['price'])}>{oldPrice} {`${currencyType}`}</div>
                </>
                ) : (
                  <div className={styles['price']}>{price}  {`${currencyType}`}</div>
                )
                }
            </div>
            ...
            <div className={styles['card-product-reviews-rating']}>
              ...
              <div className={styles['card-product-reviews']}>
                ...
                <span>{(reviewsNum && reviewsNum> 0) ? reviewsNum : '0'} отзывов</span>
              </div>
            </div>
          </div>
         </article>
